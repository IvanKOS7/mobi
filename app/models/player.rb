class Player < ApplicationRecord
  ...
  def self.top_five_players(achieve, team_id)
    Team.find(team_id).players.joins(:game_events).merge(GameEvent.where(event: achieve.event).order(count: :desc))
  end

  def self.top_world_five_players(achieve)
    Player.all.joins(:game_events).merge(GameEvent.where(event: achieve.event).order(count: :desc))
  end
end
